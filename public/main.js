...
//コメントアウトwindow.addEventListener('DOMContentLoaded', deleteOldItems(), false);
...
messageField.keypress(function (e) {
    if (e.keyCode == 13) {
        ...
        //データベースに保存する
        messagesRef.ref('/messages').push({timestamp: time, name:username, text:message});
        //メッセージ欄を空に
        messageField.val('');
...
messagesRef.ref('/messages').on('child_added', function (snapshot) {
    //取得したデータ
